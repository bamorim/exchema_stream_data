defmodule ExchemaStreamData do
  ...
  @scalar_types [T.Atom, T.Boolean, T.Date, T.DateTime, T.NaiveDateTime, T.Time, T.String, T.Number]
  ...
  defp do_gen(T.DateTime), do: map(integer(-62167219200..253402300799), &DateTime.from_unix!/1)
  ...
  } |> Map.to_list |> Enum.map(fn {mod, body} ->
    defp do_gen(unquote(mod)), do: unquote(body)
    defp do_gen(
      unquote(Module.concat(mod, Positive))
    ), do: map(unquote(body), &(1+abs(&1)))
    defp do_gen(
      unquote(Module.concat(mod, Negative))
    ), do: map(unquote(body), &(0-1-abs(&1)))
    defp do_gen(
      unquote(Module.concat(mod, NonNegative))
    ), do: map(unquote(body), &(0+abs(&1)))
    defp do_gen(
      unquote(Module.concat(mod, NonPositive))
    ), do: map(unquote(body), &(0-abs(&1)))
  end)
